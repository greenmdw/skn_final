...
class Evidence(BaseModel):
    ...
    *"반증 사례 없음"* 을 확인 판정의 근거로 쓴다.
    ...
    # 않는다 — 그래서 원문 인용을 근거로 썼다. 그런데 2026-09-08 에 **리뷰 원문을
    # 내보낼 수 없다**는 방침이 정해졌다.
    #
...
class Review(BaseModel):
    ...
    **리뷰는 주장이 아니라 품목에 달린다.** 한 품목에 주장이 여럿이면 같은 리뷰
    ...
    # 2026-09-08 방침 — 리뷰 원문을 그대로 가져와 저장하는 것은 법적 문제가 있어
    # 하지 않는다. 반드시 가공한 뒤 노출한다. 실소스 어댑터는 이 값을 요청 처리
    # 동안만 들고 있어야 하고, DB·파일에 그대로 쓰면 안 된다.
    text: str
...
class Requirement(BaseModel):
    ...
    `as_of` 가 있는 이유는 **게임사 공개 사양과 벤치마크에 시차가 있어 이미
    구형 정보인 경우가 많기 때문이다.** 기획안 §4의 대응이
