...
수정일: 2026년 9월 11일 · 개정 5 · 테이블 명세서 v6 반영

이 문서는 데이터 구조가 어떤 서비스 요구를 해결하며, 각 기록을 왜 분리했는지 설명한다. 설계 기준은 [테이블 명세서 v6](docs/db/table_spec.md)(2026년 9월 11일)다.
컬럼·타입·제약·상태값의 상세 정의와 이 문서가 다르면 테이블 명세서를 우선한다. 기존 목업과 schema-v2.json은 이전 버전 참고 자료이며 현재 설계와 동기화된 것으로 간주하지 않는다.

**현재 설계는 PostgreSQL + pgvector + 객체 저장소, 12개 스키마·58개 테이블이다.** 이번 개정은 프론트 API 계약의 비밀번호 인증, 리스트 수명주기와 확정 메타데이터, 비동기 설명 상태를
기존 책임 테이블에 반영했다. 이 문서는 구현 전 설계 설명이며 서비스 구현 완료 보고서가 아니다.
...
config.domain은 지속적인 도메인 식별자이고 config.domain_version은 질문·슬롯·검증 실행기·평가 축·속성 형식의 게시 버전이다. DB에는 검증을 마친 게시 버전만 저장하고 게시 전 초안은 버전 관리
저장소에서 관리한다. 게시된 정의를 덮어쓰지 않아야 과거 계획에 적용한 기준을 설명할 수 있다. 계획 버전은 사용한 정의 버전을 참조한다.
...
이메일·비밀번호 인증은 identity.app_user 한 행에서 관리한다. 현재 인증 수단이 하나이므로 별도 자격증명 테이블을 만들지 않되 password_hash는 명시적 인증 조회에서만 선택하고 일반 사용자 조회에는
포함하지 않는다. 비밀번호 변경·로그인·약관 동의·탈퇴는 의미가 분명한 전용 시각으로 기록하고, updated_at은 프로필·인증 상태 전반의 마지막 변경 감지에만 사용한다. 해커톤 가입은 미인증을 허용하므로
email_verified_at을 두고 인증 완료 시 기록하며 이메일 변경 시 NULL로 되돌린다. 인증 수단이 여러 개가 되면 자격증명 테이블 분리를 다시 검토한다.

프론트 API의 화면 상태를 그대로 DB에 복제하지 않는다. 리스트의 stage는 카테고리 선택, 활성 조건, 추천 실행, 확정 버전에서 계산하고 item_id는 requirement.id를 사용한다. 수량은 purchase_line.pack_count,
구매 시점은 requirement.needed_at과 timing_context, 후보 교체는 recommendation_candidate와 선택된 purchase_line으로 표현한다. budget_share·합계는 관계형 금액에서 계산한다.

리스트 삭제는 연결된 조건·추천·근거를 보존해야 하므로 planning.plan의 소프트 삭제로 처리한다. 확정 목표금액과 메모는 plan_revision에 고정하며, 실제 가격 추적 목표는 price_watch에 별도로 둔다.
추천 실행 완료와 LLM 설명 완료도 다른 상태다. recommendation_run과 recommendation_candidate에 설명 상태를 따로 두어 문장 생성 실패가 상품·가격·규칙 검사 결과를 무효화하지 않게 한다.

### 프론트 요청과 기존 구조의 중복 검토

| 프론트 개념 | 반영 위치 | 판단 |
|---|---|---|
| 이메일·비밀번호, 로그인 잠금, 동의·탈퇴 | identity.app_user 전용 컬럼 | 인증 수단이 하나인 현재는 별도 credential/consent 테이블을 만들지 않는다. 사건별 시각과 별도로 변경 감지용 updated_at, 현재 이메일의 인증 상태인 email_verified_at을 둔다. |
| 패널 폭·접힘 상태 | identity.user_preference.ui_settings | 알림 설정과 같은 사용자별 환경설정 수명주기를 가지므로 별도 테이블을 만들지 않되 JSON 계약은 분리한다. |
| 장바구니 식별·이름·최근 수정 | planning.plan | 기존 지속 식별자와 역할이 같으므로 새 list/cart 테이블을 만들지 않는다. 삭제 상태만 추가한다. |
| 화면 stage | 기존 조건·추천 실행·확정 상태에서 계산 | 중복 상태와 불일치를 만들므로 저장하지 않는다. |
| 결과 item_id·선택 수량·구매 시점 | requirement.id, purchase_line.pack_count, requirement.needed_at/timing_context | 기존 필요·구매 구조를 그대로 API 표현에 매핑한다. |
| 후보와 교체 | recommendation_candidate 및 purchase_line | 후보 이력과 현재 선택을 분리한 기존 구조를 사용한다. |
| 확정 목표금액·메모 | planning.plan_revision.target_amount/memo | 확정 스냅샷에 추가한다. 실제 추적 설정인 price_watch와 구분한다. |
| 결과 설명·후보 이유 | recommendation_run 및 recommendation_candidate | 추천 계산 상태와 문장 생성 상태를 분리한다. |
| budget_share·선택 합계·잔액 | purchase_line과 plan_revision의 금액에서 계산 | 계산 결과를 중복 저장하지 않는다. |
| 리뷰 상세·검증 근거 | evidence.review_* 및 engine.validation_* | 기존 요약·집계·검사·근거 구조로 충족하므로 새 컬럼을 추가하지 않는다. |
...
시각은 timestamptz, 금액은 numeric (18,2)와 통화 코드, 수량은 정밀 숫자형과 단위로 관리한다. 사용자 표시와 종료일 계산에는 사용자 시간대를 적용한다. 마지막 변경 시각이 필요한 테이블의
updated_at은 갱신 트리거로 관리한다. identity.app_user는 프로필·인증 상태 변경 감지를 위해 updated_at을 유지하고 identity.conversation은 생성 시각만 기록한다. 사용자 설정은 UI와 알림 JSON을 분리해 저장한다.
