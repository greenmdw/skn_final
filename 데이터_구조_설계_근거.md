...
수정일: 2026년 9월 11일 · 개정 4 · 테이블 명세서 v5 반영

이 문서는 데이터 구조가 어떤 서비스 요구를 해결하며, 각 기록을 왜 분리했는지 설명한다. 설계 기준은 [테이블 명세서 v5](docs/db/table_spec.md)(2026년 9월 11일)다.
...
포함하지 않는다. 범용 updated_at 대신 비밀번호 변경·잠금·로그인·약관 동의·탈퇴의 의미가 분명한 전용 컬럼을 둔다. 이메일 소유 확인이 끝난 주소만 계정에 확정하므로 별도 이메일 인증 상태는 두지 않는다.
인증 수단이 여러 개가 되면 자격증명 테이블 분리를 다시 검토한다.
...
| 프론트 개념 | 반영 위치 | 판단 |
|---|---|---|
| 이메일·비밀번호, 로그인 잠금, 동의·탈퇴 | identity.app_user 전용 컬럼 | 인증 수단이 하나인 현재는 별도 credential/consent 테이블을 만들지 않는다. 사건별 시각을 사용하며 범용 updated_at과 email_verified_at은 재도입하지 않는다. |
...
updated_at은 갱신 트리거로 관리한다. identity.app_user와 identity.conversation은 생성 시각만 기록하며, 사용자 설정은 알림 수신 설정만 저장한다.
